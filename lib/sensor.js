...
class Sensor
{
	...
	constructor(inverter_sn, inverter_host, inverter_port, inverter_mb_slaveid, lookup_file, protocol)
	{
		this.inverter_name = 'Homey';
		this.inverter_sn = inverter_sn;
		this.inverter = new Inverter(inverter_sn, inverter_host, inverter_port, inverter_mb_slaveid, lookup_file, protocol);
		this.getSensors();
	}
	...
	setSlaveId(inverter_mb_slaveid)
	{
		this.inverter.setSlaveId(inverter_mb_slaveid);
	}

	setProtocol(protocol)
	{
		this.inverter.setProtocol(protocol);
	}

	getProtocol()
	{
		return this.inverter.getProtocol();
	}

	setPort(port)
	{
		this.inverter.setPort(port);
	}
	...
	async getRegisterValue(registerNumber, mbFunctionCode = 3)
	{
		const result = await this.inverter.send_request(registerNumber, registerNumber, mbFunctionCode);
		console.log(`Register ${registerNumber} buffer: [${Array.from(result).join(', ')}]`);
		if (!result || result.length < 2)
		{
			throw new Error(`No data returned for register ${registerNumber} (host ${this.inverter._host}:${this.inverter._port}, slave ${this.inverter._mb_slaveid}, function code ${mbFunctionCode}, received ${result ? result.length : 0} bytes)`);
		}
		const value = result.readInt16BE(0);
		console.log(`Register ${registerNumber} readInt16BE(0) = ${value}`);
		return value;
	}
